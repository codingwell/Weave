...
class ScalaSilkParser(val buffer: IncludableInputBuffer[String]) extends Parser
{
  ...
  case object ASTExclamation extends ASTGlobalStatement
  ...
  def File = rule { "Hello" ~ WhiteSpace ~ zeroOrMore(GlobalStatement) ~~> ASTFile ~ EOI }
  
  def GlobalStatement = rule { Exclamation | TempInclude }
//FirstOf(
//			Using(file),
//			Include(),
  
  def Exclamation = rule { "!" ~ push(ASTExclamation) }
  
  def TempInclude = rule { Include ~ push(ASTInclude) }
  
  def Testing(s:ASTString, r:ASTIndexRange):Unit =
  {
    buffer.include( r.range.end, Preprocessor.StripComments( FileUtils.readAllText(s.text) ), s.text, r.range.length());
    println(s.text) 
  }
  
  def Include = rule { IncludeInternal ~>> ASTIndexRange ~~% Testing _ }
  def IncludeInternal = rule { "include" ~ WhiteSpace ~ SilkString ~ ";" }
  
  def SilkString = rule { "\"" ~ zeroOrMore(anyOf("abcdefghijklmnopqrstuvwxyz2.")) ~> ASTString ~ "\"" ~ WhiteSpace }
